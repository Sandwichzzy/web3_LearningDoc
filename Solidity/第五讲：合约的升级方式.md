...
contract UUPSContractV2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    // 移除原有的initialize函数，避免冲突
    
    function initializeV2 () public reinitializer(2) {
        value2 = 100; // 只初始化新增变量
        // 注意：不要重新初始化value，保持升级前的值
    }
...
**（部署UUPS需要部署逻辑合约和代理合约（ERC1967Proxy），然后通过代理调用初始化函数。）**
...
contract UUPSContractV1Script is Script {
    ...
    function run() public {
        ...
        // abi.encodeCall(函数签名 + 参数)  将函数调用编码为字节码 
        bytes memory data = abi.encodeCall(implementation.initialize, deployerAddress);
		
        //代理合约的构造函数会:1.将代理合约的逻辑地址设置为 implementation 
...
contract UUPSContractV2Script is Script {
    ...
    function run() public {
        ...
        Upgrades.upgradeProxy(
            ...
            abi.encodeCall(UUPSContractV2.initializeV2, ()), // 初始化数据
